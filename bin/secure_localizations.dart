...
void main() async {
  try {
    ...
    print(
        "🔑 Encryption Key: $key (${config.containsKey('key') ? 'Fixed' : 'Random Generated'})");
    ...
    // 4. Run Generator
    print("⚙️  Running localization generator...");
    ...
    print("✅ Done! Your localizations are now secured.");
  } catch (e, stack) {
    print("💥 Fatal Error: $e");
    print(stack);
  } finally {
...
''');
  print("🛠️  Created Helper: ${file.path}");
}
...
void _restoreArbFiles() {
  ...
  if (backupDir.existsSync()) {
    print("⏪ Restoring original ARB files...");
...
void _patchGeneratedFiles(String helperDir) {
  ...
  if (outputSearchDir.existsSync()) {
    print("🔎 Searching for generated files in: ${outputSearchDir.path}");
  ...
  if (targets.isEmpty) {
    print(
        "⚠️ No generated localization files found in $helperDir. Checking .dart_tool as fallback...");
  ...
  if (targets.isEmpty) {
    print(
        "❌ ERROR: Could not find any files to patch. Check your 'output_dir' in pubspec.yaml.");
    return;
  }
